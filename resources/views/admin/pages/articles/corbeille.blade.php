    ...
    <div class="p-3 w-11/12 sm:w-full my-5 mx-auto bg-gray-900 rounded-lg">
        <div class="flex">
            ...
            <div class="w-1/4 sm:1/2 my-auto text-right">
                <form action="{{route('articles.reset', $article->id)}}" method="post">
                    @csrf
                    @method('PUT')
                    <button
                    class="bg-red-500 px-3 lg:px-10  py-1 lg:py-3 mr-3 my-2 text-xs lg:text-sm shadow-sm font-semibold tracking-wider text-white rounded-lg">reset</button>
                </form>
            </div>
        </div>
